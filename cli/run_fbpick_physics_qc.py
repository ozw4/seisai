...
def _build_robust_npz_path(*, segy_path: str | Path, robust_npz_dir: str | Path) -> Path:
    return Path(robust_npz_dir) / (_build_tag(segy_path) + '.robust.npz')


def _validate_paths(cfg: dict[str, Any]) -> tuple[list[str], list[str], Path, Path, Path]:
...
def _load_vis_cfg(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(max_gathers_per_file, bool) or not isinstance(max_gathers_per_file, int):
...
def _save_vis_pngs(
    *,
    ...
    for gather_idx, (primary_key, gather_key, trace_indices) in enumerate(
        ...
        x_hw = np.asarray(info['mmap'][trace_indices], dtype=np.float32)
...
def _validate_payload_against_info(
    ...
    *,
    ...
    if n_samples_orig != int(info['n_samples']):
        ...
        raise ValueError(msg)
    if not np.isclose(dt_sec, float(info['dt_sec']), rtol=0.0, atol=1e-12):
...
def run_pipeline(config_path: str | Path) -> Path:
    ...
    segy_files, fb_files, coarse_npz_dir, robust_npz_dir, out_dir = _validate_paths(prepared)
    ...
    for segy_path, fb_path in zip(segy_files, fb_files, strict=True):
        ...
        info = _build_info(segy_path=segy_path, dataset_cfg=dataset_cfg, runtime=runtime)
        try:
