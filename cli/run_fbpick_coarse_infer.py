...
def _load_runtime() -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        validate_coarse_checkpoint_metadata=validate_coarse_checkpoint_metadata,
    )
...
def _validate_checkpoint_for_infer(
    ckpt: dict[str, Any],
    *,
    ...
    for key, value in ckpt_model_sig.items():
        if key in model_sig and model_sig[key] != value:
            expected = model_sig[key]
            msg = f'checkpoint model_sig mismatch for {key}: {value!r} != {expected!r}'
            raise ValueError(msg)
...
def _iter_single_segy_cfgs(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    out_paths = [
        _build_out_path(segy_path=segy_path, out_dir=out_dir)
...
def run_pipeline(config_path: str | Path) -> Path:
    ...
    for single_cfg in _iter_single_segy_cfgs(prepared):
        ...
        target_out_path = _build_out_path(
            segy_path=segy_path,
