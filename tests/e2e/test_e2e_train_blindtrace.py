...
def _run_e2e(*, out_dir: Path) -> tuple[Path, Path]:
	...
	base_dir = cfg.parent
	segy_paths = [str((base_dir / p).resolve()) for p in cfg_data['paths']['segy_files']]
	if len(segy_paths) != 1:
		raise ValueError('expected one segy file in test config')
	...
	cfg_data['paths']['fb_files'] = [str(fb_path)]
