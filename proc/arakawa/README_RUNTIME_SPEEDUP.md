...
own `gates` block, which overrides manifest-level scalar and list gates;
dict-valued gates are merged by key. The checked-in manifest keeps exact-match
gates on A1/A2 and uses tolerance gates only for anchor reuse or
observation-sampling stages.
