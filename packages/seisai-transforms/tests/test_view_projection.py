...
def test_project_fb_idx_view_identity_and_invalid_zero() -> None:
    H, W = 3, 10
...
def test_project_fb_idx_view_start_and_factor() -> None:
    H, W = 3, 40
...
def test_project_fb_idx_view_hflip() -> None:
    H, W = 4, 10
...
def test_project_fb_idx_view_factor_h_nearest_resample_propagates_invalid() -> None:
...
def test_project_fb_idx_view_factor_h_shrink_nearest_resample() -> None:
...
def test_project_fb_idx_view_length_mismatch_raises() -> None:
    with pytest.raises(ValueError):
        project_fb_idx_view(np.array([1, 2, 3], dtype=np.int64), H=4, W=10, meta={})


def test_project_offsets_view_identity_and_hflip() -> None:
    H = 3
...
def test_project_offsets_view_factor_h_linear_resample_expand() -> None:
...
def test_project_offsets_view_factor_h_linear_resample_shrink() -> None:
...
def test_project_offsets_view_invalid_factor_h_raises() -> None:
    with pytest.raises(ValueError):
        project_offsets_view(
...
def test_project_offsets_view_invalid_dim_raises() -> None:
    with pytest.raises(ValueError):
        project_offsets_view(np.zeros((2, 2), dtype=np.float32), H=2, meta={})


def test_project_time_view_factor_and_start() -> None:
...
def test_project_time_view_invalid_inputs_raise(time_1d, H, W, meta) -> None:
    with pytest.raises(ValueError):
