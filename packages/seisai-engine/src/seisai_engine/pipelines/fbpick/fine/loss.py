...
def _prepare_logits_target(
    *,
    ...
    expected_shape = '(B,1,1,W_local)'
    if not isinstance(logits, torch.Tensor) or int(logits.ndim) != 4:
        raise ValueError(f'{label}: logits must be torch.Tensor with shape {expected_shape}')
    if not isinstance(target, torch.Tensor) or int(target.ndim) != 4:
        raise ValueError(f'{label}: target must be torch.Tensor with shape {expected_shape}')
    ...
    if int(target.shape[1]) != 1:
        raise ValueError(f'{label}: fine target channel dim must be 1, got {int(target.shape[1])}')
    if int(logits.shape[2]) != 1:
        raise ValueError(f'{label}: fine logits trace axis must be 1, got {int(logits.shape[2])}')
    if int(target.shape[2]) != 1:
        raise ValueError(f'{label}: fine target trace axis must be 1, got {int(target.shape[2])}')
    if int(logits.shape[3]) <= 0:
        raise ValueError(f'{label}: fine local window length must be positive, got {int(logits.shape[3])}')
...
def _resolve_trace_mask(
    *,
    ...
    if not torch.equal(trace_any, trace_all):
        raise ValueError(
            f'{label}: fine probability loss only supports full-window masking along W'
        )
    if tuple(trace_any.shape) != (int(logits.shape[0]), 1):
        raise ValueError(
            f'{label}: fine trace mask must have shape ({int(logits.shape[0])},1), got {tuple(trace_any.shape)}'
        )
    return trace_any
...
def _normalize_target_prob(
    *,
    ...
    if tuple(trace_mask.shape) != (int(logits_tensor.shape[0]), 1):
        raise ValueError(
            f'{label}: trace mask shape mismatch: {tuple(trace_mask.shape)} vs ({int(logits_tensor.shape[0])},1)'
        )
    ...
    if torch.any(trace_mask & (mass_bh <= 0.0)):
        raise ValueError(
            f'{label}: selected fine windows must have positive target mass along W; '
            'enable label_valid masking or fix the fine target construction'
        )
...
def _masked_mean(
    *,
    ...
    if not isinstance(per_trace_loss, torch.Tensor) or int(per_trace_loss.ndim) != 2:
        raise ValueError(f'{label}: per-trace loss must have shape (B,1)')
    ...
    if int(selected.numel()) <= 0:
        raise ValueError(f'{label}: no fine windows selected by the loss mask')
    return selected.mean()


def _build_fine_term(
    *,
    kind: str,
    params: dict[str, Any],
    label: str,
    ...
    raise ValueError(
        f'unknown fine loss kind "{kind}"; supported: {", ".join(supported)}'
    )


def build_fine_criterion(
