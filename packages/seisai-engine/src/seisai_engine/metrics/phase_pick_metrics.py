...
def pick_argmax_w(prob_bhw: torch.Tensor) -> torch.Tensor:
    ...
    if not isinstance(prob_bhw, torch.Tensor) or int(prob_bhw.ndim) != 3:
        msg = 'prob_bhw must be torch.Tensor with shape (B,H,W)'
...
def masked_abs_error_1d(
    ...
    if not isinstance(pred_idx_bh, torch.Tensor) or int(pred_idx_bh.ndim) != 2:
        msg = 'pred_idx_bh must be torch.Tensor with shape (B,H)'
        raise ValueError(msg)
    if not isinstance(gt_idx_bh, torch.Tensor) or int(gt_idx_bh.ndim) != 2:
        msg = 'gt_idx_bh must be torch.Tensor with shape (B,H)'
        raise ValueError(msg)
...
def compute_ps_metrics_from_batch(
    ...
    *,
    ...
    if 'trace_valid' not in batch or 'label_valid' not in batch:
        msg = "batch must contain 'trace_valid' and 'label_valid'"
        raise KeyError(msg)

    meta = batch['meta']
    if not isinstance(meta, Mapping):
        msg = "batch['meta'] must be a Mapping"
        raise TypeError(msg)
    if 'p_idx_view' not in meta or 's_idx_view' not in meta:
        msg = "batch['meta'] must contain 'p_idx_view' and 's_idx_view'"
        raise KeyError(msg)
    ...
    if trace_valid.dtype is not torch.bool:
        msg = 'trace_valid must be bool'
        raise TypeError(msg)
    if label_valid.dtype is not torch.bool:
        msg = 'label_valid must be bool'
        raise TypeError(msg)

    p_gt = _as_tensor_bh(
        meta['p_idx_view'], name="meta['p_idx_view']", device=device
    ).to(dtype=torch.int64)
    s_gt = _as_tensor_bh(
        meta['s_idx_view'], name="meta['s_idx_view']", device=device
    ).to(dtype=torch.int64)
