...
def soft_label_ce_map(
    ...
    *,
    ...
    if not isinstance(target, torch.Tensor) or tuple(target.shape) != tuple(
        ...
        msg = 'target must be torch.Tensor with same shape as logits (B,C,H,W)'
        raise ValueError(
            msg
        )
    ...
    *,
    ...
    if use_trace_valid:
        if 'trace_valid' not in batch:
            msg = "batch['trace_valid'] is required when use_trace_valid=True"
            raise KeyError(msg)
    ...
    if use_label_valid:
        if 'label_valid' not in batch:
            msg = "batch['label_valid'] is required when use_label_valid=True"
            raise KeyError(msg)
    ...
    if mb is not None:
        ...
        if int(mb_t.ndim) == 3:
            ...
            if ref_h is not None and int(mb_t.shape[1]) == int(ref_h):
                ...
                # (B,W) -> (B,H,W)
                if ref_h is None:
                    msg = 'internal error: ref_h is None'
                    raise ValueError(msg)
                mb_bhw = mb_t[:, None, :].expand(
                    int(ref_b), int(ref_h), int(w_from_batch)
                )
    ...
    if ref_b is None or ref_h is None:
        msg = 'cannot infer (B,H): provide trace_valid/label_valid, or target/input, or a 3D mask_bool'
        raise ValueError(
            msg
        )
    ...
    if W is None:
        msg = 'cannot infer W: provide batch[target]/batch[input] or (B,H,W) mask_bool'
        raise ValueError(
            msg
        )
