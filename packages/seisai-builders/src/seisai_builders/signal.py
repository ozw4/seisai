...
class FBGaussMap:
    """fb_idx からガウスマップを作る。View meta（hflip/factor/start）を反映。"""
    def __init__(self, dst: str = "fb_map", sigma: float = 1.5):
        self.dst, self.sigma = dst, float(sigma)
    def __call__(self, sample: Dict[str, Any], rng=None) -> None:
        meta = sample.get("meta", {})
        fb   = sample["fb_idx"].astype(np.int64).copy()
        ...
        if np.any(fb >= 0):
            ...
            s2 = (self.sigma ** 2)
            for h, idx in enumerate(fb):
                if idx >= 0:
                    y[h] = np.exp(-0.5 * ((xs - idx) ** 2) / s2)
...
# ---------- パイプライン実行器 ----------
class BuildPlan:
    def __init__(self, wave_ops: Iterable, label_ops: Iterable,
                 input_stack: SelectStack, target_stack: SelectStack):
