...
def make_sample(H=4, W=16, dt=0.001):
	...
	fb_idx = np.array([5, -1, 8, 4], dtype=np.int64)
...
def test_fb_gauss_map_basic():
	...
	# ★H=3 に合わせて fb_idx を明示して整合とる
	s['fb_idx'] = np.array([5, -1, 8], dtype=np.int64)

	# （将来のバグ混入防止のためのワンライナー）
	assert s['fb_idx'].shape[0] == s['x_view'].shape[0]

	FBGaussMap('g', sigma=1.0)(s)
	g = s['g']
	assert g.shape == (3, 11)
	assert int(np.argmax(g[0])) == 5  # 1本目は 5 付近
	assert np.allclose(g[1], 0.0)  # 2本目は未ラベル
	assert int(np.argmax(g[2])) == 8  # 3本目は 8
