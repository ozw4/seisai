...
def test_match_input_continuous_shape():
...
def test_native_returns_same_object_size():
...
def test_invalid_fixed_params_raise():
    y = make_img()
    with pytest.raises(AssertionError):
        _ = Resizer(policy='fixed', data_kind='continuous')(y, ref=None)


def test_invalid_scale_params_raise():
    y = make_img()
    with pytest.raises(AssertionError):
