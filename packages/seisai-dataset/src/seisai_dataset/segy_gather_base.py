...
class BaseRandomSegyDataset(Dataset, abc.ABC):
	...
	def _choose_file_info(self):
		if not self.file_infos:
			raise RuntimeError('file_infos is empty')
...
class BaseSegyGatherPipelineDataset(BaseRandomSegyDataset, abc.ABC):
	...
	def _get_view_shape(
		...
		if not hasattr(x_view, 'ndim') or not hasattr(x_view, 'shape'):
			return None
		if int(getattr(x_view, 'ndim')) != 2:
			return None
		shape = getattr(x_view, 'shape')
	...
	def _try_build_sample(self, info: FileInfo, counters: dict[str, int]) -> dict | None:
	...
	@abc.abstractmethod
	def _load_fb_subset(
		self, info: FileInfo, indices: np.ndarray, sample: dict, counters: dict[str, int]
