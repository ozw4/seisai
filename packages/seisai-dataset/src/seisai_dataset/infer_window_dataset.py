...
class _NoRandRNG:
	...
	def random(self, *args, **kwargs):
		raise RuntimeError('random() is not allowed in inference')

	def uniform(self, *args, **kwargs):
		raise RuntimeError('uniform() is not allowed in inference')

	def integers(self, *args, **kwargs):
		raise RuntimeError('integers() is not allowed in inference')
...
def collate_pad_w_right(batch: Sequence[dict]) -> tuple[torch.Tensor, list[dict]]:
	...
	if not all(isinstance(x, torch.Tensor) for x in xs):
		raise TypeError("batch['input'] must be torch.Tensor")
	...
	for x in xs:
		...
		if int(x.shape[0]) != C or int(x.shape[1]) != H:
			raise ValueError('all inputs must share (C,H) for padding collate')
...
class InferenceGatherWindowsDataset(Dataset):
	...
	def __init__(
		...
		*,
		...
		if isinstance(plan, BuildPlan):
			...
			raise TypeError('plan must be BuildPlan or InputOnlyPlan')
	...
	def __getitem__(self, i: int) -> dict:
		...
		if not isinstance(x_view, np.ndarray) or x_view.ndim != 2:
			raise ValueError('transform must return 2D numpy or (2D, meta)')
		Hv, W = x_view.shape
		if int(Hv) != H:
			raise ValueError(f'transform must keep H: got {int(Hv)}, expected {H}')
		if int(W) < W0:
			raise ValueError('transform must not crop time axis in inference')
		...
		if 'input' not in sample:
			raise KeyError("plan must set sample['input']")
