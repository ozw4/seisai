...
def day_iter(start_date: str, end_date: str) -> Iterable[datetime]:
    if len(start_date) != 8 or len(end_date) != 8:
        msg = "start_date/end_date must be 'YYYYMMDD'"
...
class SeismicPUWindowDataset(Dataset):
    """Positive-unlabeled (PU) windowed seismic dataset backed by daily `.npy` files.

    This dataset builds fixed-length windows from multi-channel daily waveform arrays
    stored as NumPy files of shape (C, N), where N corresponds to
    (86400 + 2 * pad_seconds) * fs samples. Positive windows are centered relative to
    P-wave arrival timestamps (using `pre_event_sec` to place the arrival within the
    window), while unlabeled (negative) windows are sampled uniformly from time
    ranges that exclude a configurable margin around each positive event.

    Parameters
    ----------
    root_dir : str
        Directory containing the daily `.npy` files.
    csv_path : str
        CSV path containing a `P_arrival_time_local` column with timezone-aware ISO
        timestamps.
    start_date, end_date : str
        Date range in 'YYYYMMDD' (inclusive), interpreted in JST.
    win_sec : float
        Window duration in seconds.
    fs : int
        Sampling frequency in Hz.
    pre_event_sec : float, default=10.0
        Seconds before the P-arrival included in each positive window.
    neg_per_pos : int, default=1
        Number of unlabeled (negative) windows to sample per positive window.
    neg_margin_sec : float, default=120.0
        Exclusion margin (seconds) around each event when sampling negatives.
    channel_indices : list[int] | None, default=None
        Optional subset of channels to load.
    pad_seconds : int, default=30
        Padding duration on both sides of the day in the stored `.npy` files.
    filename_template : str, default='JST_{YYYYMMDD}_{fs}Hz_pad{pad}s.npy'
        Template used to construct each daily file path.
    seed : int, default=42
        RNG seed used for negative sampling.
    validate_shape : bool, default=True
        If True, validate that each `.npy` has the expected number of samples.

    Notes
    -----
    `__getitem__` returns (x_t, y_t, meta) where `x_t` is a float32 tensor of shape
    (C, win_samples), `y_t` is an int64 tensor of shape (1,), and `meta` contains
    per-window metadata such as start time and file path.
    ...
    def __init__(
        ...
        if self.win_samples <= 0:
            msg = 'win_sec * fs must be >= 1 sample'
            raise ValueError(msg)
        if not (0.0 <= self.pre_event_sec < self.win_sec):
            msg = 'pre_event_sec must satisfy 0 <= pre_event_sec < win_sec'
            raise ValueError(msg)
    ...
    def __len__(self) -> int:
        """Return the number of window items in the dataset."""
