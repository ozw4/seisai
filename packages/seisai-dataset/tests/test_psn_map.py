...
def test_psn_map_basic_contract_and_normalization():
	...
	sum_hw = y.sum(axis=0)
	np.testing.assert_allclose(sum_hw, np.ones((H, W), dtype=np.float32), atol=1e-6, rtol=0)

	label_valid = sample['label_valid']
	assert label_valid.dtype == np.bool_
	np.testing.assert_array_equal(label_valid, np.array([True, True, False], dtype=np.bool_))
