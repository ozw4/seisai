...
def smooth_1d(
	...
	if window > x.size:
		raise ValueError('window must be <= length of x')
...
def compute_window_event_probabilities(
	...
	if win_left < 0 or win_right < 0:
		raise ValueError('win_left and win_right must be >= 0')
...
def compute_event_scores(
	...
	if weights_h is None:
		...
		if weights.size != H:
			raise ValueError('weights_h length must match H')
		s = float(weights.sum())
		if s <= 0.0:
			raise ValueError('sum of weights_h must be > 0')
		weights = weights / s
