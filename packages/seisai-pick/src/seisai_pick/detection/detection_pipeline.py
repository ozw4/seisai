...
def run_event_detection_pipeline(
	...
	*,
	...
	if H == 0 or T == 0:
		raise ValueError('feature_ht must be non-empty')
	...
	if S_t.size != T:
		raise ValueError('time_support_fn must return array of shape (T,)')
	...
	if peak_indices.dtype.kind not in ('i', 'u'):
		raise TypeError(
			'peak_detector_fn must return an integer array for peak_indices'
		)
